...
exports.videoEmbedManager = exports.VideoEmbedManager = void 0;
...
class VideoEmbedManager {
        ...
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (config.videoSrc.endsWith('.m3u8')) {
                    yield this.ensureHlsLibraryLoaded();
                }
                ...
                yield this.setupVideoSource(videoElements.video, config.videoSrc, config.posterImage, videoElements.loadingIndicator);
            }
            catch (error) {
                error_handler_1.errorHandler.handleError({
     ...
     */
    createVideoContainer(videoId, config) {
        ...
        // Set size and shape
        if (isRound) {
            const diameter = Math.min(parseInt(size.width || '200'), parseInt(size.height || '158')) + 'px';
            Object.assign(container.style, {
                width: diameter,
                height: diameter,
                borderRadius: '50%'
     ...
     */
    assembleVideoEmbed(container, elements, _config // eslint-disable-line @typescript-eslint/no-unused-vars
    ) {
        // Add video
     ...
     */
    setPositionStyles(container, position, offset) {
     ...
     */
    setupVideoEventHandlers(videoId, container, elements, config) {
        const listeners = [];
        const timeouts = [];
        ...
        const ctaHandler = (e) => {
            e.stopPropagation();
            modal_manager_1.modalManager.openModalWithContent({ link: config.bookingLink });
        };
        ...
        const progressHandler = () => {
            ...
            if ((state === null || state === void 0 ? void 0 : state.isExpanded) && video.duration && video.currentTime >= 0) {
        ...
        const playHandler = () => {
            const state = this.videoStates.get(videoId);
            if (state === null || state === void 0 ? void 0 : state.isExpanded) {
                elements.progressContainer.container.style.display = 'block';
            }
        };
     ...
     */
    handleVideoContainerClick(videoId, event, elements, config) {
        const state = this.videoStates.get(videoId);
        if (!state)
            return;
        const target = event.target;
        // Don't handle clicks on interactive elements
        if (elements.closeButton.contains(target) ||
     ...
     */
    expandVideo(videoId, elements, _config) {
     ...
     */
    handleCloseButtonClick(videoId, container, elements) {
        const state = this.videoStates.get(videoId);
        if (!state)
            return;
        if (state.isExpanded) {
     ...
     */
    minimizeVideo(_videoId, container, elements) {
     ...
     */
    toggleVideoPlayPause(_videoId, elements) {
        const state = this.videoStates.get(_videoId);
        if (!state)
            return;
        if (state.isPaused) {
    ...
    getVideoConfig(_videoId) {
        ...
        return ((_a = window.meetergoSettings) === null || _a === void 0 ? void 0 : _a.videoEmbed) || null;
    }
    /**
     * Setup video source with error handling
     */
        ...
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                if (videoSrc.endsWith('.m3u8')) {
                    yield this.setupHlsVideo(video, videoSrc, posterImage, loadingIndicator);
                }
                else {
                    video.src = videoSrc;
                    video.onerror = () => {
     ...
     */
    setupHlsVideo(video, videoSrc, posterImage, loadingIndicator) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
...
exports.VideoEmbedManager = VideoEmbedManager;
// Export singleton instance
exports.videoEmbedManager = VideoEmbedManager.getInstance();
//# sourceMappingURL=video-embed-manager.js.map